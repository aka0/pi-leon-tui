...
export function extractUserText(content: unknown): string {
  ...
  return normalizePrompt(content.filter(isTextPart).map((part) => part.text).join("\n"));
}
...
export function deriveLocalTitle(prompt: unknown): string | undefined {
  const text = extractUserText(prompt).replace(/^(please\s+)?(help me\s+)?/i, "").trim();
  return sanitizeSessionName(text);
}

function isTextPart(value: unknown): value is TextPart & { type: "text"; text: string } {
  return Boolean(value && typeof value === "object" && (value as TextPart).type === "text" && typeof (value as TextPart).text === "string");
}
